...
1. 日志审计，可以自己通过nginx搭建静态文件服务器可以查看log文件【要有检索和分析功能】（包括系统审计、业务、数据库日志、防火墙日志，只需要确认能保持180天的日志即可，可以定时任务备份）
...
1. 所有服务都需要有双份备份
1. 安全保密管理员里面不能出现日志，只允许安全审计员菜单中有日志
...
1. 防火墙开通（入侵防御、病毒防御），并且防火墙规则和病毒库需离线更新到最新版本
...
1. 防火墙设置90天修改密码
1. 邮件告警设置（需连接外网），目前已设置内网，但功能不可用（形式即可）
1. 管理员口令复杂度从低调整到高、允许登录IP范围192.168.85-192.168.0.171
